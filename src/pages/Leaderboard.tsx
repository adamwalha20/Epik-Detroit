              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="font-headline text-xl font-bold text-on-surface uppercase truncate max-w-[200px]">
                    {topThree[1].profiles?.name || 'SUBJECT_PENDING'}
                  </h3>
                  <span className="font-label text-[10px] text-secondary-container tracking-widest uppercase">STABILITY SECURE</span>
                </div>
                <div className="w-full pt-4 border-t border-outline-variant/20">
                  <span className="block font-label text-[10px] text-on-surface-variant tracking-[0.1em] uppercase mb-2">SUCCESS RATE</span>
              ...
              <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <span className="font-label text-xs text-primary tracking-[0.2em] uppercase">APEX ENTITY</span>
                  </div>
                  <h3 className="font-headline text-3xl font-bold text-on-surface tracking-tight uppercase truncate max-w-[280px]">
                    {topThree[0].profiles?.name || 'SUBJECT_PENDING'}
                  </h3>
                </div>

                <div className="w-full pt-6 border-t border-primary/20">
                  <div className="flex justify-between items-end mb-2 px-2">
                    <span className="font-label text-[10px] text-on-surface-variant tracking-[0.1em] uppercase">EVALUATION COMPLETION</span>
          ...
          {topThree[2] && (
            <div className="order-3 bg-surface-container-low/40 backdrop-blur-md border border-outline-variant/20 p-6 rounded-2xl relative overflow-hidden group hover:bg-surface-container-low/60 transition-all">
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="font-headline text-xl font-bold text-on-surface uppercase truncate max-w-[200px]">
                    {topThree[2].profiles?.name || 'SUBJECT_PENDING'}
                  </h3>
                  <span className="font-label text-[10px] text-on-surface-variant tracking-widest uppercase">EVALUATION SECURE</span>
                </div>
                <div className="w-full pt-4 border-t border-outline-variant/20">
                  <span className="block font-label text-[10px] text-on-surface-variant tracking-[0.1em] uppercase mb-2">SUCCESS RATE</span>
                  <div className="h-1 w-full bg-surface-container-highest rounded-full overflow-hidden">
                ...
                <tbody className="font-body text-sm">
                  {leaders.slice(3).map((leader, index) => (
                    <tr key={index} className="border-b border-outline-variant/10 hover:bg-surface-container transition-colors">
                      ...
                      <td className="py-4 px-4 text-on-surface uppercase">
                        {leader.profiles?.name || 'SUBJECT_PENDING'}
                      </td>
